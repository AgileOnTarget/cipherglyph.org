...
| `faucet.html` | BadCoin Faucet POC page using HPP as the rate limiter, with local cold-wallet generation |
...
2026-08-22 faucet live path: `faucet.html` asks the treasury service to prepare an HPP burn, draws the burn QR, verifies the burn through the service, and then the treasury service sends 10 BAD to the entered address. The static page still holds no key and cannot sign by itself. The page also includes a browser-local BadCoin cold-wallet generator based on the BadCoin vanity generator path: fresh secp256k1 key, canonical P2SH-wrapped SegWit `B...` address, `C...` WIF, PNG download, and print-to-PDF.
