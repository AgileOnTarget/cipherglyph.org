 ...
 *
 * The address typed by the visitor is both a payload-hash160 filter and a
 * self-wallet history source. The issuer address remains a compatibility
 * source for older treasury-made Glyphs.
 */
...
function mergeTxidSources(...sources) {
  const ids = [];
  const seen = new Set();
  let truncated = false;
  for (const source of sources) {
    if (!source) continue;
    truncated = truncated || Boolean(source.truncated);
    for (const txid of source.ids || []) {
...
export async function lookupBadcoinAddress({
  ...
  try {
    const typedAddress = String(address ?? "").trim();
    const issuerHistory = await issuerTxids({ base, issuer, fetchImpl });
    const addressHistory = issuer === typedAddress
      ? null
      : await issuerTxids({ base, issuer: typedAddress, fetchImpl });
    const txids = mergeTxidSources(addressHistory, issuerHistory);
    const rows = [];
